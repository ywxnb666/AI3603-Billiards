...
class CueCardAgent(Agent):
    ...
    def __init__(self, n_l1_sims=16, use_cma=True):
        super().__init__()
        ...
        # 1. 第一层搜索 (L1 Search)
        self.n_l1_sims = n_l1_sims          # L1 模拟次数 (10 -> 24)
        # 生成候选数量上限 (生成更多候选以利用多进程，配合稀疏过滤)
        self.num_candidates_generated = 24 
        # 进入 CMA 优化的候选数量 (Top K)
        self.num_candidates_cma = 5         

        # 2. CMA-ES 深度优化
        self.use_cma = use_cma and CMA_AVAILABLE
        self.cma_maxiter = 4                # CMA-ES迭代次数
        self.cma_popsize = 8               # 每代种群大小 (5 -> 8)
        ...
        if self.use_cma:
            self.logger.info(f"[CueCard] 代理初始化完成 (CMA-ES优化已启用, {self.max_workers}-Core Profile)")
        else:
    ...
    def _play_safety(self, balls, my_targets, table):
        ...
        # 随机抽样 20 个候选进行评估，防止超时
        random.shuffle(safety_candidates)
        candidates_to_eval = safety_candidates[:20]
